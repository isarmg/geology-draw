...
- **图面**：图名、页面与横向、比例尺、字体、字号、花纹层厚；剖面图则为
  垂直夸大系数。花纹层厚默认 2.5 mm，可在 1–10 mm 间调整
...
- `--pattern-row-height-mm`：内置层状花纹的基础层厚，范围 1–10 mm，
  默认 2.5；柱状图、剖面图和图底图例同步生效
...
岩性图例块，按岩性在数据中**首次出现的顺序**从左到右、自动换行排列。
图例不再挤窄岩性描述栏，关闭时不占版面。

- **图版样框**：参照 GB/T 958—2015 表 4 正式图版，样框固定为
  **15 mm × 10 mm**、黑色方框；条目增多时只换行，不缩小或拉伸样框
- **花纹参数一致**：图例与柱内共用同一毫米制图参数和物理间距，不再
  为了凑满若干层带而临时缩放，符号宽高、方向和疏密保持一致
- **统一且可调的基础层厚**：内置层理、砖纹、波纹及斜线带默认按
  **2.5 mm** 平铺（10 mm 图例框容纳 4 层），可在“图面 › 花纹层厚”
  调为 1–10 mm；同一张图内各地层及图例始终使用同一数值，地层越厚只
  增加重复层数，不拉伸单层，也不改变真实地层厚度比例
- **完整名称**：名称在样框右侧按可用宽度完整折行，不再截掉第三行之后
  的文字；“图例项”宽度可设为 3.0–6.0 cm
- **设色说明**：图例明确说明填充色仅用于辅助区分岩性，地质含义以花纹
  与名称为准，符合标准附录 A 对按岩类设色时应说明方法的要求
- **自适应布局**：柱状图和剖面图的图底图例均按可用宽度自动换列，
  不把固定三行误称为标准要求
