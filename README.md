...
- **图面**：图名、页面与横向、比例尺、字体、字号；剖面图则为垂直夸大系数
...
- `--depth`：厚度栏改为显示地层深度；`--thick-per-layer` 逐层厚度；
  `--unit-vertical` 单位文字竖排；`--legend` 在最右侧加图例列
...
自动对齐。厚层折断只压缩被标记层，未压缩层始终保持图上标注的真实比例。
...
削截），留空即整合接触（直线）。剖面图中同一层号只需任一钻孔行标注。

### 图例列（可选）

`--legend`（GUI 勾选“栏目内容 › 图例列”）在最右侧加一列“图例”，按**岩性在
数据中出现的顺序**从上到下**均匀**列出花纹小样+名称，位置与地层
层界无关，方便读图。关闭时不占版面。

- **小样与柱内一致**：花纹以各自图形的包围盒为原点绘制（局部锚定），
  图例小样、柱状图单元格、剖面多边形看到的是同一幅花纹，不再是
  按全图平铺后被裁掉的任意一角
- **列宽自动**：未手动设“图例”栏宽时按最长岩性名取宽（仍受
  2.0–6.0 cm 夹紧）；名称过长时自动缩小字号、必要时折行，不会
  溢出列边界
- **行式瓦片自适应**：自定义的行式花纹若一个瓦片比小样还高，小样
  自动收紧比例以显示**完整一个瓦片**，各行图形都看得到
- **剖面图底部的图例按制图惯例排成三行**：各列自上而下排满三项、
  再自左而右延伸（项数不足三项时取实际项数），列宽按最长岩性名
  实测，列数多放不下时自动缩小字号；小样密度与剖面填充一致
...
表示**：在数据里加一列“**压缩**”，某层填“是”（或 Y/√）时，若该层
在图上的高度超过阈值，就把它的岩性柱段压缩到固定高度，并在中部画
一对**折断波状线**表示中间省略——**厚度栏仍标注真实厚度、深度模式
仍按真实深度累计**。默认版式会自动拉伸其余层填满图幅；对齐版式
（`--to-scale`）下整体按压缩后的显示坐标绘制。未标记的层不受影响。
